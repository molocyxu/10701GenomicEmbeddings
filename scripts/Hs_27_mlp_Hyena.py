...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Simplified MLP on HyenaDNA TF embeddings")
    parser.add_argument(
        "--data-dir",
        type=str,
...
def load_data(data_dir: Path, hyena_dir: Path, n_top_genes: int = None) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Load targets and HyenaDNA TF embeddings data."""
    print(f"[info] Loading data from {data_dir} and {hyena_dir}")
    ...
    # Load HyenaDNA TF embeddings
    emb_npy_path = hyena_dir / "hyenadna_tf_embeddings_mean.npy"
    idx_txt_path = hyena_dir / "hyenadna_tf_gene_index.txt"
    
    print(f"[info] Loading HyenaDNA embeddings from {emb_npy_path}")
    embeddings = np.load(emb_npy_path)
    
    print(f"[info] Loading TF indices from {idx_txt_path}")
    with open(idx_txt_path, 'r') as f:
        tf_ids = [line.strip() for line in f if line.strip()]
        
    if len(tf_ids) != embeddings.shape[0]:
        raise ValueError(f"Mismatch between index file lines ({len(tf_ids)}) and embedding rows ({embeddings.shape[0]})")
        
    # Create DataFrame for embeddings
    tf_emb_df = pd.DataFrame(embeddings, index=tf_ids)
    print(f"[info] Loaded TF embeddings: {tf_emb_df.shape}")
    
    # Align data - keep only TFs that exist in both datasets
    common_tfs = targets_df.index.intersection(tf_emb_df.index)
    print(f"[info] Found {len(common_tfs)} common TFs between targets and embeddings")
    
    if len(common_tfs) == 0:
        raise ValueError("No common TFs found between targets and embeddings!")
    
    # Filter and align
    targets_aligned = targets_df.loc[common_tfs]
    tf_emb_aligned = tf_emb_df.loc[common_tfs]
...
def main():
    ...
    summary = {
        ...
        "model_type": "3-layer MLP (HyenaDNA)",
